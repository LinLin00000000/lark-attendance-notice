 ...
 */
async function fetchWithAuthorization(url: string, options: RequestInit = {}, tokenProvider: () => Promise<string>): Promise<any> {
...
async function fetchWithRetry(url: string, fetchOptions: RequestInit, maxAttempts = 3) {
    let attempts = 0
    ...
    async function performFetch() {
        ...
        if (response.ok && data.code === 0) {
            ...
            throw new Error(`HTTP status ${response.status}, API code ${data.code}: ${data.msg}`)
        }
    }
    ...
    while (attempts++ < maxAttempts) {
        try {
            ...
            const isRateLimitError = error.message.includes('429') || (error.message.includes('400') && error.message.includes('99991400'))
            ...
            const retryAfter = error.response.headers.get('x-ogw-ratelimit-reset') || '10'
...
export function fetchWithTenantAuthorization(url: string, options: RequestInit = {}): Promise<any> {
    return fetchWithAuthorization(url, options, getTenantAccessToken)
}

export function fetchWithUserAuthorization(url: string, options: RequestInit = {}): Promise<any> {
...
export async function getTenantAccessToken(): Promise<string> {
    ...
    return fetchWithRetry('https://open.feishu.cn/open-apis/auth/v3/tenant_access_token/internal', {
        method: 'POST',
        headers: {
